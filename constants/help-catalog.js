...
export const HELP_SECTIONS = {
    base: {
        ...
        items: [
            ...
            { cmd: `连${D_SHOW}`, desc: '单人自动连🦌直至鹿死', quota: '逐条群播直至鹿死 · 须已转职 · 帮鹿位禁自鹿' },
    ...
    friends: {
        ...
        items: [
            ...
            { cmd: `连${D_SHOW}福@ta`, desc: '叠福层/续福回合至配额用尽（已满层只续回合）', quota: '连贴福逐条播报 · 发车位不可用' },
    ...
    playful: {
        ...
        items: [
            ...
            { cmd: `连${D_SHOW}咒@ta`, desc: '叠咒层/续咒回合至配额用尽（已满层只续回合）', quota: '叠咒过程群聊可见 · 须🦌友 · 发车位禁' },
            ...
            { cmd: `催${D_SHOW} / 催${D_SHOW}@ta`, desc: '叠催更符 buff（不改净值 · 下次自🦌兑现 +N）· 带咒者咒回合 -1', quota: `${DAILY_URGE_QUOTA} 次/日 · 无@则自催 · 可与鹿福共存` },
            { cmd: `连催${D_SHOW} / 连催${D_SHOW}@ta`, desc: '自动催至配额用尽 · 叠层催更符', quota: '连催至配额尽 · 群聊全程回显 · 无@则自催' },
            { cmd: '诈戒 / 诈戒鹿', desc: '嘴上戒🦌，实际 +1', quota: `${DAILY_FAKE_WITHDRAW_QUOTA} 次/日` },
            { cmd: '连诈戒', desc: '自动诈戒至配额用尽', quota: '假戒连发逐条可见 · 鹿车发车位不可用' },
            { cmd: `${D_SHOW}鸣`, desc: '带咒必震 1 层咒；无咒碰吉兆+1/陷阱-1 · 鹿死鸣魂', quota: `${DAILY_HOWL_QUOTA} 次/日 · 鸣魂 ${Math.round(GHOST_HOWL_KILLER_CURSE_CHANCE * 100)}%` },
            { cmd: `连${D_SHOW}鸣`, desc: '活人须带咒：逐次震咒至清；鹿死鸣魂至配额用尽', quota: '震咒逐次播报 · 咒清即停 · 发车位禁' },
            { cmd: `群${D_SHOW}溅`, desc: '日榜 Top5 各 -1 · 带咒引爆', quota: `${DAILY_GROUP_SPLASH_QUOTA} 次/日 · 溅 ${GROUP_SPLASH_TOP_N} 人 · 自损 1 · 别名：${D_SHOW}群伤 / 溅射${D_SHOW}` },
            ...
            { cmd: `连抽${D_SHOW}签`, desc: '自动抽签至配额用尽', quota: '连签过程全群可见 · 鹿车发车位不可用' },
        ],
    },
    profession: {
        title: '每日鹿职业',
        ...
        items: [
            ...
            { cmd: `看${D_SHOW}职业卡卷王 / 语姐鹿年限`, desc: '静态专精卡：配额 · 天赋详解 · 专属技 · 玩法建议', quota: '无需转职 · prebuilt 秒出 · 详查「隐藏鹿」段' },
            { cmd: '转职鹿医师 / 转职戒师 / 转职卷王 / 转职巡游 等', desc: '8 职业含向日葵鹿 · 与番外互斥 · 详见一览图', quota: '首次转职后当日锁定 · 各职业玩法次数不同' },
            { cmd: `${D_SHOW}技`, desc: '查今日专属技状态；发指令名即释放（如愈鹿@、鹿巡）', quota: '每职业 1 次/日 · 具体机制见对应职业卡' },
        ],
    },
    extraDeer: {
        title: '番外隐藏鹿',
    ...
    death: {
        ...
        items: [
            ...
            { cmd: `冥${D_SHOW}咒@ta`, desc: '亡魂对活人叠咒（同「鹿咒」机制）', quota: `${DAILY_SPECTRAL_CURSE_QUOTA} 次/日 · 勿发「冥咒」二字` },
            { cmd: `连冥${D_SHOW}咒@ta`, desc: '叠咒层/续咒回合至配额用尽', quota: '亡魂叠咒逐条播报 · 鹿死专属' },
            { cmd: `索命${D_SHOW}@ta`, desc: `有凶手须@凶手 · ${Math.round(VENGEANCE_CURSE_CHANCE * 100)}%叠咒/${Math.round(VENGEANCE_DEDUCT_CHANCE * 100)}%扣次`, quota: `${DAILY_VENGEANCE_QUOTA} 次/日 · 无凶可@替身 ${Math.round(VENGEANCE_SUBSTITUTE_CURSE_CHANCE * 100)}%叠咒` },
            { cmd: `托梦${D_SHOW}@🦌友`, desc: '对结缘🦌友：缓咒 1 回合或贴催更符', quota: `${DAILY_DREAM_QUOTA} 次/日 · 须先添加${D_SHOW}友` },
            { cmd: `连托梦${D_SHOW}@🦌友`, desc: '缓咒/贴催更符至配额用尽', quota: '托梦连发群聊可见 · 须先结缘🦌友' },
